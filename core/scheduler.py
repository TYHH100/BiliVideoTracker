...
def check_updates_job():
    ...
    # 获取冷却时间配置，提供默认值
    item_cooldown = int(settings.get('item_cooldown', 30)) # 单个监控项检查间隔，默认为30秒
    ...
    for item in monitors:
        debug_log(f"[SCHEDULER] 检查监控项: {item['name']} (类型: {item['type']}, ID: {item['remote_id']})")
        # 获取最新信息
        ...
        debug_log(f"[SCHEDULER] {item['name']} - 当前记录总数: {current_total}, B站实际总数: {remote_total}")

        # 记录检查合集的信息
        logger.info(f"检查合集: {item['name']} (类型: {item['type']}, ID: {item['remote_id']})")
        ...
        if remote_total > current_total:
            ...
            # 获取最新视频列表并添加到更新记录
            latest_videos = bili_api.get_latest_videos(item['type'], item['remote_id'], item['mid'], diff)
            debug_log(f"[SCHEDULER] {item['name']} - 获取到 {len(latest_videos)} 个最新视频")
            
            # 获取最新视频的发布时间
            update_time = datetime.datetime.now()
            if latest_videos:
                # 最新视频在列表的第一个位置（因为sort_reverse=true）
                update_time = datetime.datetime.fromtimestamp(latest_videos[0]['publish_time'])
            debug_log(f"[SCHEDULER] {item['name']} - 更新时间: {update_time}")
            
            # 构建最新视频列表的HTML内容
            videos_html = ""
            for video in latest_videos:
                video_time = datetime.datetime.fromtimestamp(video['publish_time']).strftime('%Y-%m-%d %H:%M')
            ...
            # 2. 添加视频更新记录到数据库
            for video in latest_videos:
                debug_log(f"[SCHEDULER] {item['name']} - 添加视频更新记录: {video['title']}")
            ...
            # 3. 更新数据库
            debug_log(f"[SCHEDULER] {item['name']} - 更新监控项状态，新总数: {remote_total}")
            db.update_monitor_status(item['id'], remote_total, int(datetime.datetime.now().timestamp()))
            has_update = True
    ...
    if batch_updates and settings.get('smtp_enable') == '1':
        ...
        debug_log(f"[SCHEDULER] 统一推送更新通知 - 合集数: {len(batch_updates)}, 总视频数: {total_updates}")
    ...
    global_cooldown = int(settings.get('global_cooldown', 1200))  # 使用与实际调度相同的默认值1200秒
    ...
    debug_log(f"[SCHEDULER] 更新下次检查时间为: {next_time.strftime('%Y-%m-%d %H:%M:%S')}")
...
def start_scheduler():
    ...
    scheduler.add_job(daily_log_maintenance_job, 'cron', hour=0, minute=0, id='daily_log_maintenance_job')
    ...
    if settings.get('monitor_active') == '1':
        ...
        scheduler.add_job(check_updates_job, 'interval', seconds=global_cooldown, id='monitor_job')
        # 立即运行一次
...
def check_single_monitor(monitor_id):
    ...
    debug_log(f"[SCHEDULER] 检查监控项: {monitor['name']} (类型: {monitor['type']}, ID: {monitor['remote_id']})")
    ...
    remote_total = info['total']
    debug_log(f"[SCHEDULER] {monitor['name']} - 当前记录总数: {current_total}, B站实际总数: {remote_total}")
    
    # 更新数据库中的总数
    db.update_monitor_status(monitor['id'], remote_total, int(datetime.datetime.now().timestamp()))
    ...
    if remote_total > current_total:
        ...
        # 获取最新视频列表并添加到更新记录
        latest_videos = bili_api.get_latest_videos(monitor['type'], monitor['remote_id'], monitor['mid'], diff)
        debug_log(f"[SCHEDULER] {monitor['name']} - 获取到 {len(latest_videos)} 个最新视频")
        
        # 添加视频更新记录到数据库
        for video in latest_videos:
            debug_log(f"[SCHEDULER] {monitor['name']} - 添加视频更新记录: {video['title']}")
            success, msg = db.add_video_update(
                monitor['id'],
                video['video_id'],
                video['title'],
