...
class BiliAPI:
    ...
    def _get(self, url, mid_for_referer, max_retries=3):
        ...
        debug_log("BiliAPI", "_get", f"准备发送请求: URL={url}, Referer={headers['Referer']}, max_retries={max_retries}")
        
        for retry in range(max_retries):
            try:
                debug_log("BiliAPI", "_get", f"发送第 {retry+1}/{max_retries} 次请求: {url}")
                resp = requests.get(url, headers=headers, timeout=10)
                debug_log("BiliAPI", "_get", f"收到响应: status_code={resp.status_code}, URL={url}")
                
                if resp.status_code == 200:
                    debug_log("BiliAPI", "_get", f"请求成功: {url}")
                    return resp.json()
                else:
                    logger.error(f"API请求失败: 状态码 {resp.status_code} for URL {url}, retry {retry+1}/{max_retries}")
                    debug_log("BiliAPI", "_get", f"请求失败: status_code={resp.status_code}, retry={retry+1}/{max_retries}, URL={url}")
            except requests.RequestException as e:
                logger.error(f"API请求异常: {e} for URL {url}, retry {retry+1}/{max_retries}")
                debug_log("BiliAPI", "_get", f"请求异常: {e}, retry={retry+1}/{max_retries}, URL={url}")
    ...
    def get_info(self, monitor_type, remote_id, mid):
        """统一入口：根据类型获取信息"""
        debug_log("BiliAPI", "get_info", f"获取监控信息: type={monitor_type}, remote_id={remote_id}, mid={mid}")
        ...
        try:
            if monitor_type == 'series':
                debug_log("BiliAPI", "get_info", f"调用 _get_series 获取系列信息: series_id={remote_id}")
                return self._get_series(remote_id, mid)
            elif monitor_type == 'season':
                debug_log("BiliAPI", "get_info", f"调用 _get_season 获取合集信息: season_id={remote_id}")
                return self._get_season(remote_id, mid)
        except Exception as e:
            logger.error(f"获取监控信息失败: {e}, type={monitor_type}, remote_id={remote_id}, mid={mid}")
            debug_log("BiliAPI", "get_info", f"获取监控信息失败: {e}, type={monitor_type}, remote_id={remote_id}, mid={mid}")
    ...
    def _get_series(self, series_id, mid):
        ...
        debug_log("BiliAPI", "_get_series", f"获取系列信息: series_id={series_id}, URL={url}")
        
        data = self._get(url, mid)
        
        if data and data['code'] == 0 and 'data' in data and 'meta' in data['data']:
            debug_log("BiliAPI", "_get_series", f"成功获取系列信息: series_id={series_id}, data={data}")
            meta = data['data']['meta']
        ...
        debug_log("BiliAPI", "_get_series", f"未获取到有效系列信息: series_id={series_id}, data={data}")
        return None
    ...
    def _get_season(self, season_id, mid):
        ...
        debug_log("BiliAPI", "_get_season", f"获取合集信息: season_id={season_id}, mid={mid}, URL={url}")
        
        data = self._get(url, mid)
        
        if data and data['code'] == 0 and 'data' in data:
            debug_log("BiliAPI", "_get_season", f"成功获取合集信息: season_id={season_id}, data={data}")
            if 'meta' in data['data']:
                ...
                if 'archives' in data['data'] and data['data']['archives']:
                    try:
                        last_update = max(v.get('pubdate', 0) for v in data['data']['archives'])
                        debug_log("BiliAPI", "_get_season", f"获取到最后更新时间: {last_update}")
                    except (TypeError, ValueError) as e:
                        debug_log("BiliAPI", "_get_season", f"获取最后更新时间失败: {e}")
                        last_update = 0
        ...
        debug_log("BiliAPI", "_get_season", f"未获取到有效合集信息: season_id={season_id}, data={data}")
        return None
    ...
    def get_latest_videos(self, monitor_type, remote_id, mid, count=5):
        ...
        count: 获取的视频数量
        """
        debug_log("BiliAPI", "get_latest_videos", f"获取最新视频: type={monitor_type}, remote_id={remote_id}, mid={mid}, count={count}")
        ...
        if monitor_type == 'season':
            ...
            if data and data['code'] == 0 and 'data' in data and 'archives' in data['data']:
                debug_log("BiliAPI", "get_latest_videos", f"成功获取合集最新视频: type={monitor_type}, data={data}")
                archives = data['data']['archives']
                videos = []
                for i, archive in enumerate(archives):
                    debug_log("BiliAPI", "get_latest_videos", f"处理视频 {i+1}/{len(archives)}: archive={archive}")
                    # 确保所有必要字段都存在
                    if not archive:
                    ...
                    if not video_id:
                        debug_log("BiliAPI", "get_latest_videos", f"跳过无video_id的视频")
                        continue
                    video = {
                        'video_id': str(video_id),
                        'title': archive.get('title', '未命名视频'),
                        'publish_time': archive.get('pubdate', 0),
                        'cover': archive.get('pic', '')  # API返回的是pic字段，不是cover
                    }
                    videos.append(video)
                    debug_log("BiliAPI", "get_latest_videos", f"添加视频: {video}")
                debug_log("BiliAPI", "get_latest_videos", f"共获取到 {len(videos)} 个合集最新视频")
                return videos
            ...
            if data and data['code'] == 0 and 'data' in data and 'archives' in data['data']:
                debug_log("BiliAPI", "get_latest_videos", f"成功获取系列最新视频: type={monitor_type}, data={data}")
                archives = data['data']['archives']
                videos = []
                for i, archive in enumerate(archives):
                    debug_log("BiliAPI", "get_latest_videos", f"处理视频 {i+1}/{len(archives)}: archive={archive}")
                    ...
                    videos.append(video)
                    debug_log("BiliAPI", "get_latest_videos", f"添加视频: {video}")
                debug_log("BiliAPI", "get_latest_videos", f"共获取到 {len(videos)} 个系列最新视频")
                return videos
        
        debug_log("BiliAPI", "get_latest_videos", f"未获取到最新视频: type={monitor_type}, remote_id={remote_id}")
        return []
    ...
    def parse_url(self, url):
        ...
        try:
            parsed = urlparse(url)
            debug_log("BiliAPI", "parse_url", f"解析后的URL: scheme={parsed.scheme}, netloc={parsed.netloc}, path={parsed.path}, query={parsed.query}")
            ...
            if mid and remote_id and m_type in ['series', 'season']:
                debug_log("BiliAPI", "parse_url", f"解析成功: mid={mid}, remote_id={remote_id}, type={m_type}")
                return mid, remote_id, m_type
            else:
                debug_log("BiliAPI", "parse_url", f"解析失败: mid={mid}, remote_id={remote_id}, type={m_type}")
