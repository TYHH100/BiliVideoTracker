...
def _get_encryption_key():
    ...
    if os.path.exists(KEY_FILE):
        ...
        with open(KEY_FILE, "rb") as f:
            key = f.read()
        debug_log(f"[DB] 加载加密密钥成功")
        return key
    else:
        # 生成新密钥
        key = Fernet.generate_key()
        # 确保只有当前用户可读写
        os.chmod(DATA_DIR, 0o700)  # 设置数据目录权限
        with open(KEY_FILE, "wb") as f:
            f.write(key)
        os.chmod(KEY_FILE, 0o600)  # 设置密钥文件权限
        debug_log(f"[DB] 生成新的加密密钥")
        return key
...
def decrypt_data(encrypted_data):
    ...
    except (InvalidToken, cryptography.fernet.InvalidToken):
        # 如果解密失败，返回原始数据（可能已经是明文或无效的加密数据）
        logger.warning("[DB] 解密失败: 无效的令牌，返回原始数据")
...
def verify_token(raw_token):
    ...
    try:
        ...
        if not result:
            time.sleep(1)  # 校验失败时强制延迟 1 秒，防止每秒数万次的暴力破解
        return result
    except Exception as e:
        ...
        traceback.print_exc()
        time.sleep(1)  # 出错时也延迟，防止通过错误信息枚举
        return False
    finally:
        token_pool.return_connection(conn)
...
def get_all_settings():
    ...
    try:
        ...
        for row in c.fetchall():
            key = row["key"]
            value = row["value"]
            # 对email_auth_code进行解密（只有非空值才尝试解密）
            if key == "email_auth_code" and value:
                try:
                    value = decrypt_data(value)
                except Exception:
                    # 如果解密失败，保持原值（可能是未加密的或无效的）
                    pass
            settings[key] = value
        return settings
    finally:
...
def update_setting(key, value):
    ...
    try:
        c = conn.cursor()
        # 对email_auth_code进行加密
        if key == "email_auth_code":
