...
def init_dbs():
    ...
    try:
        ...
        c.execute('''CREATE TABLE IF NOT EXISTS auth_token (
    ...
    try:
        ...
        c.execute('''CREATE TABLE IF NOT EXISTS monitor_list (
        ...
        try:
            ...
            # 定义所有应该存在的字段
            expected_columns = {
                'id', 'mid', 'remote_id', 'type', 'name', 'cover',
                'total_count', 'last_check_ts', 'up_mid', 'desc', 'is_active', 'archived'
            }
            ...
            for column in missing_columns:
                ...
                if column == 'total_count':
                    c.execute('ALTER TABLE monitor_list ADD COLUMN total_count INTEGER DEFAULT 0')
                elif column == 'last_check_ts':
                    c.execute('ALTER TABLE monitor_list ADD COLUMN last_check_ts INTEGER DEFAULT 0')
                elif column == 'up_mid':
                    c.execute('ALTER TABLE monitor_list ADD COLUMN up_mid TEXT')
                elif column == 'desc':
                    c.execute('ALTER TABLE monitor_list ADD COLUMN desc TEXT')
                elif column == 'is_active':
                    c.execute('ALTER TABLE monitor_list ADD COLUMN is_active INTEGER DEFAULT 1')
                elif column == 'archived':
                    c.execute('ALTER TABLE monitor_list ADD COLUMN archived INTEGER DEFAULT 0')
                else:
                    # 默认添加TEXT类型字段
        ...
        c.execute('''CREATE TABLE IF NOT EXISTS video_updates (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            monitor_id INTEGER,
            video_id TEXT,
            video_title TEXT,
        ...
        try:
            ...
            # 定义所有应该存在的字段
            expected_columns = {
                'id', 'monitor_id', 'video_id', 'video_title', 'publish_time', 'cover'
            }
            ...
            for column in missing_columns:
                ...
                if column == 'monitor_id':
                    ...
                    c.execute('ALTER TABLE video_updates ADD COLUMN publish_time INTEGER DEFAULT 0')
                else:
                    # 默认添加TEXT类型字段
...
def set_token(raw_token):
    ...
    try:
        ...
        c.execute("INSERT INTO auth_token (id, token_hash, created_at) VALUES (1, ?, ?)", (hashed, now))
        conn.commit()
...
def get_token_info():
    ...
    try:
        ...
        row = c.fetchone()
        if row:
            return {'hash_preview': row[0][:8] + '...', 'created_at': row[1], 'exists': True}
...
def add_monitor(data):
    """添加新的监控项"""
    debug_log(f"[DB] 添加监控项: remote_id={data['remote_id']}, type={data['type']}, name={data['name']}")
    conn = data_pool.get_connection()
    try:
        c = conn.cursor()
        # 查重
        debug_log(f"[DB] 检查监控项是否已存在: remote_id={data['remote_id']}, type={data['type']}")
        c.execute("SELECT id FROM monitor_list WHERE remote_id=? AND type=?", (data['remote_id'], data['type']))
        if c.fetchone():
            debug_log(f"[DB] 监控项已存在: remote_id={data['remote_id']}, type={data['type']}")
            return False, "已存在该监控项"
        
        debug_log(f"[DB] 插入监控项数据")
        c.execute('''INSERT INTO monitor_list 
            (mid, remote_id, type, name, cover, total_count, last_check_ts, up_mid, desc, is_active)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''',
            (data['mid'], data['remote_id'], data['type'], data['name'], 
             data['cover'], data['total'], int(datetime.datetime.now().timestamp()), 
             data['mid'], data['desc'], 1)  # 默认激活
        )
        conn.commit()
...
def update_monitor_active_status(monitor_id, is_active):
    """更新监控项的激活状态"""
    debug_log(f"[DB] 更新监控项激活状态: monitor_id={monitor_id}, is_active={is_active}")
    conn = data_pool.get_connection()
    try:
        c = conn.cursor()
        c.execute("UPDATE monitor_list SET is_active=? WHERE id=?", (is_active, monitor_id))
        conn.commit()
        debug_log(f"[DB] 监控项激活状态更新成功")
        return True, "状态更新成功"
    except Exception as e:
        logger.error(f"更新监控项状态失败: {e}, monitor_id={monitor_id}, is_active={is_active}")
...
def update_monitor_status(monitor_id, new_total, timestamp):
    ...
    try:
        c = conn.cursor()
        debug_log(f"[DB] 更新监控项状态: monitor_id={monitor_id}, new_total={new_total}, timestamp={timestamp}")
        c.execute("UPDATE monitor_list SET total_count=?, last_check_ts=? WHERE id=?", 
...
def update_monitor_archived_status(monitor_id, archived):
    ...
    try:
        ...
        # 归档时同时暂停监控
        if archived:
            c.execute("UPDATE monitor_list SET archived=?, is_active=0 WHERE id=?", (archived, monitor_id))
        else:
            c.execute("UPDATE monitor_list SET archived=? WHERE id=?", (archived, monitor_id))
        conn.commit()
        debug_log(f"[DB] 监控项归档状态更新成功")
        return True, "状态更新成功"
    except Exception as e:
        logger.error(f"更新监控项归档状态失败: {e}, monitor_id={monitor_id}, archived={archived}")
        return False, f"更新失败: {str(e)}"
    finally:
        data_pool.return_connection(conn)
...
def add_video_update(monitor_id, video_id, video_title, publish_time, cover):
    """添加视频更新记录"""
    debug_log(f"[DB] 添加视频更新记录: monitor_id={monitor_id}, video_id={video_id}, title={video_title}")
    conn = data_pool.get_connection()
    try:
        c = conn.cursor()
        # 先检查是否已存在相同的视频记录
        ...
        c.execute('''INSERT INTO video_updates 
        ...
        c.execute('''SELECT id, cover FROM video_updates 
                     ORDER BY publish_time DESC LIMIT -1 OFFSET ?''', (limit,))
        ...
        if old_records:
            ...
            # 删除旧记录
            c.execute(f"DELETE FROM video_updates WHERE id IN ({','.join(['?']*len(old_ids))})", old_ids)
...
def get_recent_updates(limit=None):
    ...
    try:
        c = conn.cursor()
        debug_log(f"[DB] 获取最近更新视频记录，限制{limit if limit is not None else '默认'}")
        ...
        c.execute('''SELECT vu.*, ml.name as monitor_name FROM video_updates vu 
                     JOIN monitor_list ml ON vu.monitor_id = ml.id 
                     ORDER BY vu.publish_time DESC LIMIT ?''', (limit,))
...
def get_monitor_update_stats(monitor_id):
    ...
    try:
        ...
        c.execute('''SELECT publish_time FROM video_updates 
                     WHERE monitor_id = ? 
                     ORDER BY publish_time DESC''', (monitor_id,))
...
def get_batch_monitor_update_stats(monitor_ids):
    ...
    try:
        ...
        c.execute(f'''SELECT monitor_id, publish_time FROM video_updates 
                     WHERE monitor_id IN ({placeholders}) 
                     ORDER BY monitor_id, publish_time DESC''', tuple(monitor_ids))
...
def _calculate_update_stats(monitor_id, publish_times):
    ...
    if len(publish_times) < 2:
        # 如果视频数量不足2个，无法计算间隔
        debug_log(f"[DB] 视频数量不足，无法计算更新间隔: monitor_id={monitor_id}, {len(publish_times)}个视频")
        return {
    ...
    for i in range(len(publish_times) - 1):
        interval_days = (publish_times[i] - publish_times[i + 1]) / (24 * 3600)  # 转换为天
