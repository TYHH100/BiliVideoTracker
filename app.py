...
if getattr(sys, "frozen", False):
    ...
    app = Flask(
        "BiliVideoTracker", template_folder=template_folder, static_folder=static_folder
    )
else:
...
def add_monitor_item():
    ...
    if not mid or not remote_id:
        return (
            jsonify({"code": 400, "msg": "无法解析该链接，请确保是系列或合集链接"}),
            400,
        )
    ...
    if success:
        ...
        if new_monitor_id:
            threading.Thread(
                target=check_single_monitor, args=(new_monitor_id,), daemon=True
            ).start()
...
def get_settings():
    ...
    # 脱敏处理
    return jsonify({"code": 0, "config": settings, "token": token_info})
...
def reset_token():
    ...
    if token_info["exists"]:
        if not check_auth():
            return jsonify({"code": 401, "msg": "需要先登录才能重置"}), 401
...
def proxy_image():
    ...
    try:
        ...
        if not content_type or not content_type.startswith("image/"):
            logger.error(
                f"获取的内容不是图片: {image_url}, Content-Type: {content_type}"
            )
        ...
        try:
            ...
            debug_log(
                f"准备转换图片为webp格式，原始Content-Type: {content_type}, webp缓存文件: {webp_cache_filename}"
            )
...
def upload_background():
    ...
    try:
        ...
        if (
            ...
            return (
                jsonify(
                    {
                        "code": 1,
                        "msg": "不支持的文件类型，仅支持PNG、JPG、JPEG、GIF、WEBP格式",
                    }
                ),
                400,
            )
        ...
        # 生成唯一文件名
        filename = (
            f'background_{uuid.uuid4().hex}.{file.filename.rsplit(".", 1)[1].lower()}'
        )
